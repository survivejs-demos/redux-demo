  ...
  switch (action.type) {
    ...
    case types.DETACH_FROM_LANE:
      return state.updateIn(
        ...
        lane =>
          lane.deleteIn(["notes", lane.get("notes").indexOf(action.noteId)])
      );
    ...
    case types.MOVE:
      ...
      const sourceLane = state.findEntry(
        lane => lane.get("notes").indexOf(sourceId) >= 0
      );
      const targetLane = state.findEntry(
        lane => lane.get("notes").indexOf(targetId) >= 0
      );
      ...
      return state
        ...
        .updateIn(
          ...
          [targetLane[0]],
          lane =>
            lane.updateIn(["notes"], notes =>
              notes.insert(targetNoteIndex, sourceId)
            )
        );
